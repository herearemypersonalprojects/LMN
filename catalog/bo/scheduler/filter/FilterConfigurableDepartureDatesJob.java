 ...
 */
public class FilterConfigurableDepartureDatesJob extends FilterDepartureDatesJob {

    /** The logger. */
    private static final Logger LOGGER = Logger.getLogger(FilterConfigurableDepartureDatesJob.class);
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        if (LOGGER.isInfoEnabled()) {
            LOGGER.info("In FilterConfigurableDepartureDatesJob class");
        }
        ...
        try {
            ...
            if (tosToFilter != null) {
                String[] toAndDepOffSets = tosToFilter.split(SEPARATOR);
                if (toAndDepOffSets != null) {
                	for (String toAndDepOffSetArray : toAndDepOffSets) {
                        ArrayList<String> productIds = new ArrayList<String>();
                		if (toAndDepOffSetArray != null)  {
                			String[] toAndDepOffSet = toAndDepOffSetArray.split("\\|");
                			if (toAndDepOffSet != null) {
                				criteria.setTourOperatorCode(new String[] {toAndDepOffSet[0]});
                				this.setMinAndMaxDepartureDates(criteria, Integer.parseInt(toAndDepOffSet[1]));
