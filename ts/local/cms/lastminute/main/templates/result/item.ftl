<#assign
  ...
  s_fin = context.lookup("s_fin")!'0'
  s_lin = context.lookup("s_lin")!'0'
/>

<#if s_fin != '0'>
  <#assign nbExpertList = 0 />
</#if>

<#if nbExpertList <= adsIndex>
  <#if productIndex - 1 == adsIndex - nbExpertList>
    ...
    <div class="colLeft">
      ...
      <#if '' != productDisplayable.imageUrl>
        <div class="image-wrap">
          <a href="/${productUrl}">
            <img src="${productDisplayable.imageUrl}" alt="${productDisplayable.title}">
          </a>
          <#if (productDisplayable.getPicto())?has_content>
            <img src="${productDisplayable.picto.code}" alt="${productDisplayable.picto.label}" class="lastMinute">
          </#if>
        </div>
      </#if>
